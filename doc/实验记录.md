...
​		波形没啥大问题，但是数值结果是正确的数值结果的1/16，**数值结果不太对，而且数据打包不正确（检测结果与波形匹配不上）**。
...
（1）行为级仿真：用来检查代码中的语法错误以及代码行为的正确性。
...
（3）时序仿真：IDE加入考虑器件的延时等等，比较真实的接近运行情况。


  * **综合**后时序仿真，**比较**真实的接近真实情况。

    ![image-20231029144344358](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20231029144344358.png)

    信号结果完全不对，比如会出现下面这种情况：
    ...
    ![image-20231031134703033](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20231031134703033.png)

    读rom数据的时候，数据比时钟上升沿延迟了1.018ns。

    

    打开时序报告查看，确实存在很多问题：

    ![image-20231029150728127](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20231029150728127.png)
